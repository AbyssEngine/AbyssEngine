...
struct dc6 *dc6_load(const char *path) {
    ...
    struct mpq_stream *stream = fileman_load(path);
    mpq_stream_read(stream, &result->header, 0, sizeof(struct dc6_header));
    ...
    result->frames = malloc(sizeof(struct dc6_frame) * total_frames);
    FAIL_IF_NULL(result->frames);

    for (uint32_t i = 0; i < total_frames; i++) {
        struct dc6_frame *frame = &result->frames[i];
        mpq_stream_read(stream, &frame->header, 0, sizeof(struct dc6_frame_header));
...
void dc6_decode_frame(const struct dc6 *dc6, uint32_t frame_index) {
    struct dc6_frame *frame = &dc6->frames[frame_index];
