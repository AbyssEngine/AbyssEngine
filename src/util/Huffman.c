...
struct LinkedNode *huffman_decode(struct BitReader *BitReader, struct LinkedNode *head) {
    ...
    while (node->child_0 != NULL) {
        int bit = bit_reader_read_bit(BitReader);
        if (bit == -1) {
...
uint8_t *huffman_decompress(uint8_t *buffer, uint32_t buffer_len, uint32_t *result_size) {
    ...
    struct LinkedNode *tail     = huffman_build_list(primes[comp_type]);
    struct LinkedNode *head     = huffman_build_tree(tail);
    *result_size                = 0;
    uint8_t          *result    = calloc(0, sizeof(uint8_t));
    struct BitReader *BitReader = bit_reader_init(buffer, buffer_len);
    int               decoded;

    bool do_loop = true;
    while (do_loop) {
        struct LinkedNode *node = huffman_decode(BitReader, head);
        ...
        case DECOMP_VAL_1:
            ...
            break;
        case DECOMP_VAL_2: {
            uint8_t new_value         = bit_reader_read_bits(BitReader, 8);
            *result_size             += 1;
